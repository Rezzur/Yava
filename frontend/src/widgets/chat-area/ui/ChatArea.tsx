...
export function ChatArea() {
  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>, fileType: 'IMAGE' | 'DOCUMENT') => {
    const file = e.target.files?.[0];
    if (!file || !chatId) return;

    setIsSending(true);
    try {
      // Pass both file and its name
      const uploadRes = await fileApi.upload(file, file.name);
      await sendMessage(Number(chatId), file.name, fileType, uploadRes.data.url);
    } catch (error) {
  ...
  const startRecording = async () => {
    try {
      ...
      mediaRecorder.onstop = async () => {
        ...
        try {
          // Explicitly pass name for voice message
  ...
  return (
    <div className="flex-1 flex h-full relative z-10 bg-[#e4ebf0] dark:bg-slate-950 overflow-hidden">
      ...
      <input type="file" ref={documentInputRef} accept="*/*" className="hidden" onChange={(e) => handleFileChange(e, 'DOCUMENT')} />
      ...
      <div className="flex-1 flex flex-col h-full relative">
        ...
        <div className="h-16 bg-white dark:bg-slate-900 border-b border-slate-200 dark:border-slate-800 flex items-center justify-between px-6 z-10 shrink-0 shadow-sm">
          {!isSearchOpen ? (
            ...
            <div className="flex items-center w-full"><Search className="w-5 h-5 text-slate-400 mr-4" /><input type="text" placeholder="Поиск сообщений..." className="flex-1 bg-transparent outline-none text-[16px] dark:text-slate-100" autoFocus /></div>
          )}
              ...
              <div key={msg.id} className={clsx("flex w-full mb-1", isMe ? "justify-end" : "justify-start")}>
                <div className={clsx("flex max-w-[75%]", isMe ? "flex-row-reverse" : "flex-row")}>
                  ...
                  <div className={clsx("rounded-2xl px-3 py-2 text-[15px] shadow-sm relative group min-w-[80px]", isMe ? "bg-[#eeffde] dark:bg-[#2b5278] text-black dark:text-white rounded-br-none ml-2" : "bg-white dark:bg-[#182533] text-black dark:text-white rounded-bl-none mr-2")}>
                    ...
                    {msg.type?.toUpperCase() === 'DOCUMENT' && msg.mediaUrl && <a href={API_URL + msg.mediaUrl} target="_blank" rel="noreferrer" className="flex items-center gap-3 bg-black/5 dark:bg-white/5 p-3 rounded-xl mb-2 hover:bg-black/10 transition-colors"><div className="w-10 h-10 rounded-full bg-blue-500 flex items-center justify-center text-white"><FileText className="w-5 h-5" /></div><div className="flex-1 overflow-hidden"><div className="text-sm font-medium truncate">{msg.text}</div><div className="text-[10px] opacity-50 uppercase">Документ</div></div></a>}
        ...
        <div className="p-4 z-10 bg-transparent flex justify-center w-full max-w-3xl mx-auto">
          <div className="flex items-end gap-2 w-full relative">
            <div className="flex-1 bg-white dark:bg-slate-900 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-800 flex items-end p-2 px-3 transition-colors relative">
              {!isRecording ? (
                <>
                  <button type="button" onClick={() => setIsEmojiOpen(!isEmojiOpen)} className="p-2 text-slate-400 hover:text-[#3390ec]"><Smile className="w-6 h-6" /></button>
                  ...
                  <div className="relative" ref={attachMenuRef}>
                    <button type="button" onClick={() => setIsAttachOpen(!isAttachOpen)} className="p-2 text-slate-400 hover:text-[#3390ec]"><Paperclip className="w-6 h-6" /></button>
                    {isAttachOpen && (
