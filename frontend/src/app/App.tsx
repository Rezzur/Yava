 ...
 * * Отвечает за маршрутизацию верхнего уровня, защиту приватных маршрутов (auth guard)
 * и базовую структуру интерфейса (Layout).
 * * Ключевые функции:
 * - Проверка статуса авторизации (`isAuthenticated`, `token`).
 * - Автоматический редирект неавторизованных пользователей на страницу `/login`.
 * - Первичная загрузка данных пользователя (`fetchCurrentUser`) и списка чатов (`fetchChats`)
 * при успешной авторизации.
 * - Отрисовка основной оболочки приложения (Sidebar + область контента) для авторизованных пользователей.
 * * @returns {JSX.Element} Возвращает страницу авторизации, компонент редиректа `<Navigate>`
 * или основную структуру приложения с настроенными маршрутами `<Routes>`.
 */
function App() {
